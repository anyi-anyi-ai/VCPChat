...
ipcMain.on('start-speech-recognition', (event) => {
    // 寻找发送请求的窗口（可能是主窗口，也可能是语音聊天窗口）
    let targetWindow = null;
    if (mainWindow && event.sender === mainWindow.webContents) {
        targetWindow = mainWindow;
    } else {
        targetWindow = openChildWindows.find(win => win.webContents === event.sender);
    }

    if (!targetWindow) {
        console.error('[Main] Received start-speech-recognition from unknown sender.');
        return;
    }
