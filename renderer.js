...
// ========== VCP Interactive Button Fix (CSP Bypass via Event Delegation) ==========
// Issue: Electron CSP blocks inline onclick="input('...')" attributes.
// Solution: Global event delegation that intercepts button clicks and manually
//           extracts the intended text from the onclick attribute, then fills
//           the input box and triggers send.
// Patch by: 爱弥斯 (Aemeath) - VCP Agent
// Date: 2026-03-12
// ============================================================================
(function() {
    document.addEventListener('click', function(e) {
        ...
        // Match patterns: input('text'), input("text"), input(`text`)
        const match = onclickAttr.match(/input\s*\(\s*(['"`])([\s\S]*?)\1\s*\)/);
        if (!match) return;
        ...
        if (inputEl && sendBtn) {
            ...
            if (window.uiHelperFunctions && window.uiHelperFunctions.autoResizeTextarea) {
                window.uiHelperFunctions.autoResizeTextarea(inputEl);
            }
    ...
    }, true); // Use capture phase to intercept before CSP blocks
    
    // Also expose window.input as a fallback for non-CSP environments
    ...
    console.log('[VCP Interactive Button Fix] Event delegation initialized successfully.');
})();